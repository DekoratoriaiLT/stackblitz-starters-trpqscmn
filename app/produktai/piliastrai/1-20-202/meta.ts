...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.20.202') || getProduct('1-20-202');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Pilaster 1.20.202 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-20-202`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-20-202`;
