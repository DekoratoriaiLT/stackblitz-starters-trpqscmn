...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.20.101') || getProduct('1-20-101');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Pilaster 1.20.101 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-20-101`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-20-101`;
