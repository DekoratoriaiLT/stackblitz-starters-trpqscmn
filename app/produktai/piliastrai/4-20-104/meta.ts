...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.20.104') || getProduct('4-20-104');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Pilaster 4.20.104 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-20-104`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-20-104`;
