...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.20.303') || getProduct('1-20-303');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Pilaster 1.20.303 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-20-303`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-20-303`;
