...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.33.301') || getProduct('4-33-301');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Frizas 4.33.301 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-33-301`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-33-301`;
