...
const CATEGORY = 'balustrados-porankiai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.72.101') || getProduct('4-72-101');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Balustrados Porankis 4.72.101 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-72-101`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-72-101`;
