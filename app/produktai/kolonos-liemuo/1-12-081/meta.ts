...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.12.081') || getProduct('1-12-081');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 1.12.081 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-12-081`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-12-081`;
