...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.22.030') || getProduct('1-22-030');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 1.22.030 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-22-030`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-22-030`;
