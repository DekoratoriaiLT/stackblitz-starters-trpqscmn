...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.12.301') || getProduct('4-12-301');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 4.12.301 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-12-301`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-12-301`;
