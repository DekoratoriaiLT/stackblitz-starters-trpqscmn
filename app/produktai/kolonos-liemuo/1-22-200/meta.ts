...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.22.200') || getProduct('1-22-200');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 1.22.200 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-22-200`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-22-200`;
