...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-16-011`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.16.011') || await getProduct('1-16-011');

  if (!product) {
    return <div>Produktas nerastas (1.16.011)</div>;
  }
