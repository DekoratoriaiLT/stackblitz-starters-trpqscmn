...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-22-050`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.22.050') || await getProduct('1-22-050');

  if (!product) {
    return <div>Produktas nerastas (1.22.050)</div>;
  }
