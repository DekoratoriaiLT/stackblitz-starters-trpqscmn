...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-12-002`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.12.002') || await getProduct('4-12-002');

  if (!product) {
    return <div>Produktas nerastas (4.12.002)</div>;
  }
