...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-16-080`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.16.080') || await getProduct('1-16-080');

  if (!product) {
    return <div>Produktas nerastas (1.16.080)</div>;
  }
