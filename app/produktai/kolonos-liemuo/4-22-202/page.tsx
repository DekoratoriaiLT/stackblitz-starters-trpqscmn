...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-22-202`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.22.202') || await getProduct('4-22-202');

  if (!product) {
    return <div>Produktas nerastas (4.22.202)</div>;
  }
