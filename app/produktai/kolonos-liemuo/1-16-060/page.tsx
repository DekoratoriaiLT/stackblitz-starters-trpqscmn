...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-16-060`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.16.060') || await getProduct('1-16-060');

  if (!product) {
    return <div>Produktas nerastas (1.16.060)</div>;
  }
