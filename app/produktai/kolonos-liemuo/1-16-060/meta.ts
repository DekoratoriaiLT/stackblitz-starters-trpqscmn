...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.16.060') || getProduct('1-16-060');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 1.16.060 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-16-060`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-16-060`;
