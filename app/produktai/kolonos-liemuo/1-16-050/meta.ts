...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.16.050') || getProduct('1-16-050');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column shaft 1.16.050 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-16-050`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-16-050`;
