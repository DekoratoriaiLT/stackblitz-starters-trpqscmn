...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-16-101`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.16.101') || await getProduct('4-16-101');

  if (!product) {
    return <div>Produktas nerastas (4.16.101)</div>;
  }
