...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-203`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.10.203') || await getProduct('1-10-203');

  if (!product) {
    return <div>Produktas nerastas (1.10.203)</div>;
  }
