...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.10.203') || getProduct('1-10-203');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 1.10.203 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-10-203`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-203`;
