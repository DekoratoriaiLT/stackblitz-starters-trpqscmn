...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-213`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.10.213') || await getProduct('1-10-213');

  if (!product) {
    return <div>Produktas nerastas (1.10.213)</div>;
  }
