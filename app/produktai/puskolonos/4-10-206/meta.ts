...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.10.206') || getProduct('4-10-206');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 4.10.206 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-10-206`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-10-206`;
