...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-10-307`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.10.307') || await getProduct('4-10-307');

  if (!product) {
    return <div>Produktas nerastas (4.10.307)</div>;
  }
