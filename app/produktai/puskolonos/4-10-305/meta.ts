...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.10.305') || getProduct('4-10-305');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 4.10.305 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-10-305`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-10-305`;
