...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-211`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.10.211') || await getProduct('1-10-211');

  if (!product) {
    return <div>Produktas nerastas (1.10.211)</div>;
  }
