...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.10.302') || getProduct('1-10-302');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 1.10.302 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-10-302`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-302`;
