...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.10.210') || getProduct('1-10-210');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 1.10.210 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-10-210`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-210`;
