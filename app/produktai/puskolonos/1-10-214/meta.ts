...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.10.214') || getProduct('1-10-214');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Half column 1.10.214 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-10-214`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-10-214`;
