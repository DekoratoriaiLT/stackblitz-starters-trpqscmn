...
const CATEGORY = 'balustrados-pagrindai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.74.201') || getProduct('4-74-201');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Balustrados Pagrindas 4.74.201 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-74-201`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-74-201`;
