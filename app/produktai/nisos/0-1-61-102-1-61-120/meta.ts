...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('0.1.61.102.1.61.120') || getProduct('0-1-61-102-1-61-120');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Niche 0-1.61.102-1.61.120 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/0-1-61-102-1-61-120`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/0-1-61-102-1-61-120`;
