...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('0.1.61.101.1.61.110') || getProduct('0-1-61-101-1-61-110');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Niche 0-1.61.101-1.61.110 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/0-1-61-101-1-61-110`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/0-1-61-101-1-61-110`;
