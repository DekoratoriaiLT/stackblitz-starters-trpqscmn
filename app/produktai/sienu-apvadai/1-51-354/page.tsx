...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-51-354`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.51.354') || await getProduct('1-51-354');

  if (!product) {
    return <div>Produktas nerastas (1.51.354)</div>;
  }
