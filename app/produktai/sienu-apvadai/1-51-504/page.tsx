...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-51-504`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.51.504') || await getProduct('1-51-504');

  if (!product) {
    return <div>Produktas nerastas (1.51.504)</div>;
  }
