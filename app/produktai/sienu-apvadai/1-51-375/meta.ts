...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.51.375') || getProduct('1-51-375');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Sienos Apvadas 1.51.375 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-51-375`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-51-375`;
