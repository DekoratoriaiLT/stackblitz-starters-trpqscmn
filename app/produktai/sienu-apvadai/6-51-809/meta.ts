...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.51.809') || getProduct('6-51-809');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Sienos Apvadas 6.51.809 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-51-809`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-51-809`;
