...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/6-51-809`;
...
export default async function Page() {
  ...
  const product = await getProduct('6.51.809') || await getProduct('6-51-809');

  if (!product) {
    return <div>Produktas nerastas (6.51.809)</div>;
  }
