...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.71.101') || getProduct('4-71-101');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Balustrada 4.71.101 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-71-101`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-71-101`;
