...
const CATEGORY = 'riejamieji-elementai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.55.005') || getProduct('1-55-005');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Keystone 1.55.005 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-55-005`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-55-005`;
