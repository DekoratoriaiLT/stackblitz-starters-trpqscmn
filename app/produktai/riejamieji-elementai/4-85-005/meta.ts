...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.85.005') || getProduct('4-85-005');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Keystone 4.85.005 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-85-005`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-85-005`;
