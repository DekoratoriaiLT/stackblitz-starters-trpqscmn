...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.54.040') || getProduct('1-54-040');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.54.040 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-54-040`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-54-040`;
