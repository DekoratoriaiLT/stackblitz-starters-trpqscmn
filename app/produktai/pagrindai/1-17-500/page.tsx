...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-17-500`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.17.500') || await getProduct('1-17-500');

  if (!product) {
    return <div>Produktas nerastas (1.17.500)</div>;
  }
