...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.17.600') || getProduct('1-17-600');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.17.600 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-17-600`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-17-600`;
