...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-13-001`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.13.001') || await getProduct('4-13-001');

  if (!product) {
    return <div>Produktas nerastas (4.13.001)</div>;
  }
