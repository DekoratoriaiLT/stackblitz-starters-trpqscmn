...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.13.001') || getProduct('4-13-001');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 4.13.001 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-13-001`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-13-001`;
