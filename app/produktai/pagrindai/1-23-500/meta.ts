...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.23.500') || getProduct('1-23-500');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.23.500 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-23-500`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-23-500`;
