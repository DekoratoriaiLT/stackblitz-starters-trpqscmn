...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-54-050`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.54.050') || await getProduct('1-54-050');

  if (!product) {
    return <div>Produktas nerastas (1.54.050)</div>;
  }
