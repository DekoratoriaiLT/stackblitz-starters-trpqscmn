...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.54.050') || getProduct('1-54-050');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.54.050 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-54-050`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-54-050`;
