...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.17.201') || getProduct('4-17-201');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 4.17.201 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-17-201`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-17-201`;
