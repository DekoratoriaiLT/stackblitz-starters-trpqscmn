...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-23-100`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.23.100') || await getProduct('1-23-100');

  if (!product) {
    return <div>Produktas nerastas (1.23.100)</div>;
  }
