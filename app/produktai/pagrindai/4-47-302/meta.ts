...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.47.302') || getProduct('4-47-302');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 4.47.302 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-47-302`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-47-302`;
