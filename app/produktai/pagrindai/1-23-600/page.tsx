...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-23-600`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.23.600') || await getProduct('1-23-600');

  if (!product) {
    return <div>Produktas nerastas (1.23.600)</div>;
  }
