...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.54.070') || getProduct('1-54-070');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.54.070 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-54-070`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-54-070`;
