...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.13.900') || getProduct('1-13-900');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.13.900 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-13-900`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-13-900`;
