...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-13-900`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.13.900') || await getProduct('1-13-900');

  if (!product) {
    return <div>Produktas nerastas (1.13.900)</div>;
  }
