...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-23-300`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.23.300') || await getProduct('1-23-300');

  if (!product) {
    return <div>Produktas nerastas (1.23.300)</div>;
  }
