...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.23.200') || getProduct('1-23-200');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.23.200 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-23-200`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-23-200`;
