...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-17-302`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.17.302') || await getProduct('4-17-302');

  if (!product) {
    return <div>Produktas nerastas (4.17.302)</div>;
  }
