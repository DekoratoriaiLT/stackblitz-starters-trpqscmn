...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.13.700') || getProduct('1-13-700');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Base 1.13.700 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-13-700`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-13-700`;
