...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-13-100`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.13.100') || await getProduct('1-13-100');

  if (!product) {
    return <div>Produktas nerastas (1.13.100)</div>;
  }
