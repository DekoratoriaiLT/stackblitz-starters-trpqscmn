...
const CATEGORY = 'lango-angokrastai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.88.001') || getProduct('4-88-001');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Window jamb 4.88.001 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-88-001`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-88-001`;
