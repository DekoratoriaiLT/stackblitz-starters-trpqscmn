...
const CATEGORY = 'arkiniai-elementai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.61.511') || getProduct('1-61-511');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Arkinis Elementas 1.61.511 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-61-511`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-61-511`;
