...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.30.214') || getProduct('1-30-214');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 1.30.214 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-30-214`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-30-214`;
