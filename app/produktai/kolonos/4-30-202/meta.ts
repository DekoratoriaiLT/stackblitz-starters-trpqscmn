...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.30.202') || getProduct('4-30-202');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 4.30.202 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-30-202`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-30-202`;
