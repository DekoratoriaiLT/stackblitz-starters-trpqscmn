...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.30.105') || getProduct('1-30-105');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 1.30.105 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-30-105`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-30-105`;
