...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.30.304') || getProduct('4-30-304');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 4.30.304 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-30-304`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-30-304`;
