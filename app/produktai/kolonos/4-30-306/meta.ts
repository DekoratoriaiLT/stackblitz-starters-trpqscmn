...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.30.306') || getProduct('4-30-306');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 4.30.306 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-30-306`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-30-306`;
