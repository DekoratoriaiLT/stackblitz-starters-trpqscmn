...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.30.102') || getProduct('1-30-102');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 1.30.102 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-30-102`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-30-102`;
