...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.30.103') || getProduct('1-30-103');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 1.30.103 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-30-103`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-30-103`;
