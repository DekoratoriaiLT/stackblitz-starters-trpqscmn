...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.30.204') || getProduct('4-30-204');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Column 4.30.204 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-30-204`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-30-204`;
