...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.155') || getProduct('1-50-155');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.155 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-155`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-155`;
