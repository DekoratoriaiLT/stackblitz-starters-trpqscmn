...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.160') || getProduct('1-50-160');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.160 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-160`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-160`;
