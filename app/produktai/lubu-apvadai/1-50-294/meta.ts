...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.294') || getProduct('1-50-294');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.294 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-294`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-294`;
