...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.50.801') || getProduct('6-50-801');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 6.50.801 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-50-801`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-50-801`;
