...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.81.001') || getProduct('4-81-001');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 4.81.001 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-81-001`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-81-001`;
