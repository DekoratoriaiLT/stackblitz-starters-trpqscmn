...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.102') || getProduct('1-50-102');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.102 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-102`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-102`;
