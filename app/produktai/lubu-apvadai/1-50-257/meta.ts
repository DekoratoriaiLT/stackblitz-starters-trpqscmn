...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.257') || getProduct('1-50-257');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.257 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-257`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-257`;
