...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.173') || getProduct('1-50-173');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.173 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-173`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-173`;
