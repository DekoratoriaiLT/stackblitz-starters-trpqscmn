...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/6-50-802`;
...
export default async function Page() {
  ...
  const product = await getProduct('6.50.802') || await getProduct('6-50-802');

  if (!product) {
    return <div>Produktas nerastas (6.50.802)</div>;
  }
