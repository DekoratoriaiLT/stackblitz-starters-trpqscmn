...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.172') || getProduct('1-50-172');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.172 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-172`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-172`;
