...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.50.804') || getProduct('6-50-804');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 6.50.804 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-50-804`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-50-804`;
