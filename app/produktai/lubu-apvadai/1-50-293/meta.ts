...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.293') || getProduct('1-50-293');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.293 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-293`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-293`;
