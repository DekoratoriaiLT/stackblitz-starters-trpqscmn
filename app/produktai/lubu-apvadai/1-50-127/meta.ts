...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.127') || getProduct('1-50-127');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.127 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-127`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-127`;
