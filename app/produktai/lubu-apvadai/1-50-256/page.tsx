...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-256`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.50.256') || await getProduct('1-50-256');

  if (!product) {
    return <div>Produktas nerastas (1.50.256)</div>;
  }
