...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-31-301`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.31.301') || await getProduct('4-31-301');

  if (!product) {
    return <div>Produktas nerastas (4.31.301)</div>;
  }
