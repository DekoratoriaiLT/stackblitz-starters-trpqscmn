...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-01-101`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.01.101') || await getProduct('4-01-101');

  if (!product) {
    return <div>Produktas nerastas (4.01.101)</div>;
  }
