...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.273') || getProduct('1-50-273');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.273 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-273`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-273`;
