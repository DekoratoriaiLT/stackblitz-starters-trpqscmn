...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.131') || getProduct('1-50-131');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.131 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-131`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-131`;
