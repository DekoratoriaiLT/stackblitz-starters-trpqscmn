...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-115`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.50.115') || await getProduct('1-50-115');

  if (!product) {
    return <div>Produktas nerastas (1.50.115)</div>;
  }
