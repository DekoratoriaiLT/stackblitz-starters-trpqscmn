...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.171') || getProduct('1-50-171');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.171 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-171`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-171`;
