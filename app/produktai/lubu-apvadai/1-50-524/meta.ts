...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.50.524') || getProduct('1-50-524');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 1.50.524 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-50-524`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-50-524`;
