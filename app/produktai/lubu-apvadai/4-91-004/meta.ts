...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.91.004') || getProduct('4-91-004');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Karnizas 4.91.004 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-91-004`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-91-004`;
