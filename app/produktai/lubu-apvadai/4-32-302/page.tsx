...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-32-302`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.32.302') || await getProduct('4-32-302');

  if (!product) {
    return <div>Produktas nerastas (4.32.302)</div>;
  }
