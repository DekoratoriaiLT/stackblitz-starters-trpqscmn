...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-91-001`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.91.001') || await getProduct('4-91-001');

  if (!product) {
    return <div>Produktas nerastas (4.91.001)</div>;
  }
