...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.52.286') || getProduct('1-52-286');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Apvadų kampas 1.52.286 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-52-286`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-52-286`;
