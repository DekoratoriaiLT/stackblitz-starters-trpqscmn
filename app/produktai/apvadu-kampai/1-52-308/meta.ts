...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.52.308') || getProduct('1-52-308');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Apvadų kampas 1.52.308 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-52-308`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-52-308`;
