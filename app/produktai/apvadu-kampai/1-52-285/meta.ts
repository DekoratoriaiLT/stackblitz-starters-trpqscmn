...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.52.285') || getProduct('1-52-285');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Apvadų kampas 1.52.285 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-52-285`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-52-285`;
