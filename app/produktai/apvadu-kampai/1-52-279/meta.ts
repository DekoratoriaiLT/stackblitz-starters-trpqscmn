...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.52.279') || getProduct('1-52-279');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Apvadų kampas 1.52.279 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-52-279`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-52-279`;
