...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.87.033') || getProduct('4-87-033');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Arch frame 4.87.033 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-87-033`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-87-033`;
