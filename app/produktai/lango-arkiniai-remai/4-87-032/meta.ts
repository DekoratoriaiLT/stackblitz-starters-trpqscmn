...
const CATEGORY = 'lango-arkiniai-remai';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.87.032') || getProduct('4-87-032');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Arch frame 4.87.032 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-87-032`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-87-032`;
