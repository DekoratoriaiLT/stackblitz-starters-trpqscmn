...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.59.904') || getProduct('1-59-904');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative panel 1.59.904 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-59-904`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-59-904`;
