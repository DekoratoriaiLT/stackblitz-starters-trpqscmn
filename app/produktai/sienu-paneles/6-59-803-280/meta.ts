...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.59.803.280') || getProduct('6-59-803-280');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative panel 6.59.803-280 6.59.803.280 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-59-803-280`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-59-803-280`;
