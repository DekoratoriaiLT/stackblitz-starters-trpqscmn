...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.59.802') || getProduct('6-59-802');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative panel 6.59.802 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-59-802`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-59-802`;
