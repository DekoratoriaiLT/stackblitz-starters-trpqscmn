...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.59.801') || getProduct('6-59-801');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative panel 6.59.801 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-59-801`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-59-801`;
