...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.86.003') || getProduct('4-86-003');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Bossage 4.86.003 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-86-003`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-86-003`;
