...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.19.137') || getProduct('1-19-137');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Corbel 1.19.137 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-19-137`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-19-137`;
