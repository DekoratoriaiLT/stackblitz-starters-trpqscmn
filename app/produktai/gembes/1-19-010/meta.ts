...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.19.010') || getProduct('1-19-010');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Corbel 1.19.010 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-19-010`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-19-010`;
