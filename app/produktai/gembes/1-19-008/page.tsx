...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-19-008`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.19.008') || await getProduct('1-19-008');

  if (!product) {
    return <div>Produktas nerastas (1.19.008)</div>;
  }
