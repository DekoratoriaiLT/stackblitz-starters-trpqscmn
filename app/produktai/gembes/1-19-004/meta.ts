...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.19.004') || getProduct('1-19-004');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Corbel 1.19.004 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-19-004`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-19-004`;
