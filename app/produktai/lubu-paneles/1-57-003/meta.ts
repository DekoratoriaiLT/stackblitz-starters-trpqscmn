...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.57.003') || getProduct('1-57-003');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling panel 1.57.003 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-57-003`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-57-003`;
