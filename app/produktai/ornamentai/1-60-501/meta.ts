...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.60.501') || getProduct('1-60-501');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 1.60.501 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-60-501`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-60-501`;
