...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.60.135') || getProduct('1-60-135');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 1.60.135 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-60-135`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-60-135`;
