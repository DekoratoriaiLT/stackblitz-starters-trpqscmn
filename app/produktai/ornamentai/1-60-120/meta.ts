...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.60.120') || getProduct('1-60-120');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 1.60.120 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-60-120`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-60-120`;
