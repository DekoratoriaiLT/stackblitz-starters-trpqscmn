...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.60.017') || getProduct('1-60-017');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 1.60.017 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-60-017`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-60-017`;
