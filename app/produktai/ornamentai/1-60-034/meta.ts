...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.60.034') || getProduct('1-60-034');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 1.60.034 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-60-034`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-60-034`;
