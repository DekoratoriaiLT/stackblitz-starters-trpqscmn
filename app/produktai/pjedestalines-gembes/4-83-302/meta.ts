...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.83.302') || getProduct('4-83-302');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Pedestal corbel 4.83.302 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-83-302`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-83-302`;
