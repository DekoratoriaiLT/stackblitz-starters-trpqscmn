...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.56.049') || getProduct('1-56-049');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling rose 1.56.049 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-56-049`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-56-049`;
