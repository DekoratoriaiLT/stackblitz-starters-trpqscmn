...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.56.511') || getProduct('1-56-511');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling rose 1.56.511 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-56-511`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-56-511`;
