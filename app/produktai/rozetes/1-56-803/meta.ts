...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.56.803') || getProduct('1-56-803');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling rose 1.56.803 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-56-803`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-56-803`;
