...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.56.044') || getProduct('1-56-044');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling rose 1.56.044 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-56-044`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-56-044`;
