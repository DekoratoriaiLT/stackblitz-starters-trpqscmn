...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.56.041') || getProduct('1-56-041');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ceiling rose 1.56.041 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-56-041`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-56-041`;
