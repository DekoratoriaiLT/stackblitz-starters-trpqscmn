...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.15.300') || getProduct('1-15-300');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ring 1.15.300 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-15-300`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-15-300`;
