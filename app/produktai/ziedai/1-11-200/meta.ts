...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.11.200') || getProduct('1-11-200');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Ring 1.11.200 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-11-200`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-11-200`;
