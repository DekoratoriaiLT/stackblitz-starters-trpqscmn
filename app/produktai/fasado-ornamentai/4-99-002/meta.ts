...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.99.002') || getProduct('4-99-002');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Fasado Ornamentas 4.99.002 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-99-002`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-99-002`;
