...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.21.006') || getProduct('1-21-006');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Capital 1.21.006 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-21-006`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-21-006`;
