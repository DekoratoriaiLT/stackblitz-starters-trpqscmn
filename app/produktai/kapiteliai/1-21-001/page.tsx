...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-21-001`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.21.001') || await getProduct('1-21-001');

  if (!product) {
    return <div>Produktas nerastas (1.21.001)</div>;
  }
