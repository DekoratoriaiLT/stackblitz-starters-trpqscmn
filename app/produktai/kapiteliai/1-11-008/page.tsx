...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-11-008`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.11.008') || await getProduct('1-11-008');

  if (!product) {
    return <div>Produktas nerastas (1.11.008)</div>;
  }
