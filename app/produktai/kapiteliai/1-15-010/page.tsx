...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-15-010`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.15.010') || await getProduct('1-15-010');

  if (!product) {
    return <div>Produktas nerastas (1.15.010)</div>;
  }
