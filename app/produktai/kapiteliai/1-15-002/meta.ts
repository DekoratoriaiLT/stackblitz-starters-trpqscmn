...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.15.002') || getProduct('1-15-002');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Capital 1.15.002 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-15-002`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-15-002`;
