...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.21.005') || getProduct('1-21-005');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Capital 1.21.005 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-21-005`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-21-005`;
