...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/4-51-201`;
...
export default async function Page() {
  ...
  const product = await getProduct('4.51.201') || await getProduct('4-51-201');

  if (!product) {
    return <div>Produktas nerastas (4.51.201)</div>;
  }
