...
const generateProductJsonLd = (product: any) => {
  ...
  const productUrl = `${BASE_URL}/produktai/${CATEGORY}/1-21-002`;
...
export default async function Page() {
  ...
  const product = await getProduct('1.21.002') || await getProduct('1-21-002');

  if (!product) {
    return <div>Produktas nerastas (1.21.002)</div>;
  }
