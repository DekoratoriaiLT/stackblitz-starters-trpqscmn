...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.15.301') || getProduct('4-15-301');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Capital 4.15.301 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-15-301`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-15-301`;
