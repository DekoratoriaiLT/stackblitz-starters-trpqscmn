...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.84.053') || getProduct('4-84-053');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Platband 4.84.053 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-84-053`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-84-053`;
