...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('4.84.052') || getProduct('4-84-052');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Platband 4.84.052 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/4-84-052`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/4-84-052`;
