...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.54.014') || getProduct('1-54-014');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Platband 1.54.014 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-54-014`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-54-014`;
