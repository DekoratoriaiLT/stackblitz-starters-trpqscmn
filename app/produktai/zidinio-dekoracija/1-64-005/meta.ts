...
const CATEGORY = 'zidinio-dekoracija';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.64.005') || getProduct('1-64-005');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative mantelpiece elements 1.64.005 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-64-005`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-64-005`;
