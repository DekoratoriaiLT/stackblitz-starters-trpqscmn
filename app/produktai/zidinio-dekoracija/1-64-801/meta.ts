...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.64.801') || getProduct('1-64-801');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Decorative mantelpiece elements 1.64.801 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-64-801`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-64-801`;
