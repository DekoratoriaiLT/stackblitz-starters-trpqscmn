...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.53.109') || getProduct('1-53-109');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Grindu Apvadas 1.53.109 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-53-109`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-53-109`;
