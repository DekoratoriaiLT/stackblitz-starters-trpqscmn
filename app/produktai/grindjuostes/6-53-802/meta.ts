...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('6.53.802') || getProduct('6-53-802');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Grindu Apvadas 6.53.802 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/6-53-802`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/6-53-802`;
