...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const product = getProduct('1.53.807') || getProduct('1-53-807');

  if (!product) {
    // Fallback metadata — still indexable
    return {
      title: 'Grindu Apvadas 1.53.807 | Interjero ir Fasado Dekoratoriai',
      ...
      alternates: {
        canonical: `${BASE_URL}/produktai/${CATEGORY}/1-53-807`,
      },
    };
  }

  const canonicalUrl = `${BASE_URL}/produktai/${CATEGORY}/1-53-807`;
